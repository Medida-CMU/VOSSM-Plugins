...
")
    ...
    ok <- readline(paste("Is it OK to change .RProfile and collect anonymous usage reports? (y/n) ", collapse="", sep=""))
...
scimapRegister <- function(deps, thetime, quiet = FALSE) {
    ...
    if (isEnabledScimap()) {
        library(RJSONIO)
        exportJson <- toJSON(scimapPacket(deps, thetime))
        fileName <- paste("/home/yesyayen/MonitorData/RMonitorData/data_", as.integer( Sys.time() ), ".json",sep="")
        write(exportJson, fileName,append=TRUE)
        # result = tryCatch({
